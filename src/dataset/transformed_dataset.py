...
class MultiStreamingLeRobotDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        # R2-D17 fix: include the DISTRIBUTED RANK in the seed — without it,
        # every DDP rank draws the identical weighted stream (duplicate samples
        # across the whole job). Rank is resolved from torch.distributed when
        # initialized, falling back to the RANK env var (accelerate sets it).
        _rank = 0
        try:
            ...
            if _dist.is_available() and _dist.is_initialized():
                ...
                import os as _os
                _rank = int(_os.environ.get("RANK", "0") or 0)
        except Exception:
            _rank = 0
        if worker_info is None:
            rng_seed = self.seed + _rank * 100_003
        else:
            rng_seed = self.seed + _rank * 100_003 + worker_info.id
