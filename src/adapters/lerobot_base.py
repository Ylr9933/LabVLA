...
class LeRobotAdapterBase(BaseAdapter):
    ...
    def __getitem__(self, flat_idx: int) -> dict:
        ...
        for col in df.columns:
            ...
            if col in image_keys_set:
                ...
                _img_tf = getattr(self, "image_transforms", None)
                if _img_tf is not None:
                    _apply_seeded = getattr(_img_tf, "apply_with_seed", None)
                    if callable(_apply_seeded):
                        frame_t = _apply_seeded(
                            frame_t,
                            seed_parts=(self.repo_id, ep_idx, col, frame_in_ep),
                        )
                    else:
                        frame_t = _img_tf(frame_t)
                out[_canonical_image_key[col]] = frame_t
                continue
        ...
        for key, deltas in dt_items:
            ...
            # UR/Festo raw 11, Rizon4 raw 12 → first 8 dims) and
    ...
    _ZERO_FRAME_REASON_SLOTS: tuple[str, ...] = (
        ...
        "frame_overshoot",
        "missing_episode",
        "schema_camera_missing",
        "video_metadata_missing",
        "other",
    )

    def _init_zero_frame_shared(self) -> None:
        """Create the fork-shared fallback counter. Call from subclass __init__
        (main process, before any DataLoader worker forks). Best-effort: on
        platforms without usable shared memory the local Counter still works
        for num_workers=0."""
        try:
            import multiprocessing as _mp
            self._zero_frame_shared = _mp.Array("l", len(self._ZERO_FRAME_REASON_SLOTS))
        except Exception:
            self._zero_frame_shared = None
    ...
    def _zero_frame(
        ...
        if shared is not None:
            ...
            idx = slots.index(reason) if reason in slots else len(slots) - 1
            try:
    ...
    def video_fallback_summary(self) -> dict:
        ...
        if shared is not None:
            try:
                ...
                out = {
                    reason: int(v)
                    for reason, v in zip(self._ZERO_FRAME_REASON_SLOTS, vals)
                    if int(v) > 0
                }
                if out:
                    return out
