...
from transformers.models.qwen3_vl import Qwen3VLForConditionalGeneration, Qwen3VLConfig
...
def _bundled_vlm_config_path(path: str):
    """Return the path to a bundled Qwen3-VL architecture config (`vlm_config.json`)
    if `path` is a self-contained checkpoint dir that carries one, else None.

    Self-contained checkpoints place the Qwen3-VL config + tokenizer + processor files
    FLAT in pretrained_model/ (the arch config renamed to `vlm_config.json` to avoid
    colliding with LabVLA's own `config.json`), with NO Qwen weights. Presence of
    `vlm_config.json` is the signal to build the backbone weightlessly from it (the
    checkpoint's model.safetensors then fills the weights). A real Qwen dir / HF id has
    no `vlm_config.json` -> the legacy from_pretrained path is used.
    """
    import os
    if not path:
        return None
    cand = os.path.join(path, "vlm_config.json")
...
class LabVLAModel(nn.Module):
    ...
    def __init__(self, config: LabVLAConfig):
        ...
        # Self-contained checkpoints bundle the Qwen3-VL config + tokenizer + processor
        # files FLAT in pretrained_model/ (the arch config renamed to vlm_config.json to
        # avoid colliding with LabVLA's own config.json), with NO Qwen weights. When such
        # a bundle is present (vlm_config.json found next to vlm_pretrained_path), build
        # the backbone architecture from it (empty init); the LabVLA checkpoint's
        # model.safetensors then fills every weight on the subsequent load. Numerically
        # identical to from_pretrained+overlay (golden test: 0.0 max-abs-diff over all VLM
        # params — tie_word_embeddings=True ties embed_tokens<->lm_head, and the only
        # non-persistent buffers (rope inv_freq) are recomputed in __init__), and it skips
        # loading the 8.9GB Qwen weights that from_pretrained would load and discard.
        _bundled_vlm_cfg = _bundled_vlm_config_path(config.vlm_pretrained_path)
        if _bundled_vlm_cfg is None:
            ...
            assert getattr(_vlm_cfg, "tie_word_embeddings", False), (
                "weightless VLM init requires tie_word_embeddings=True; got "
                f"{getattr(_vlm_cfg, 'tie_word_embeddings', None)} at {_bundled_vlm_cfg}"
            )
            ...
            logger.info(
                "[self-contained] built Qwen3-VL backbone weightlessly from "
                f"{_bundled_vlm_cfg}; weights come from the LabVLA checkpoint"
            )
