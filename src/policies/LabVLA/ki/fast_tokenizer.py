...
Local path (default, R2-M4 doc fix): /all-flash-data/Embodied_models/fast —
configured via --fast_tokenizer_path / LABVLA_FAST_TOKENIZER_PATH; the wrapper
itself takes the path explicitly.
...
class FastTokenizerWrapper:
    ...
    def __init__(self, path: str, vocab_size: int, max_length: int):
        ...
        # R2-D18: fail loud at LOAD time when the processor's own vocab differs
        # from the configured one. The KI head's nn.Embedding is sized by
    ...
    def encode(self, actions: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        if L > 0:
            ...
            mask[:L] = True
            # R2-D18: per-encode id-range guard (cheap max over <=max_length
            # ints). Catches any id escaping the load-time vocab check (e.g. a
            # processor without a vocab_size attribute) BEFORE it reaches the
            # KI embedding as an opaque device-side assert.
