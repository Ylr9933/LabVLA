...
class PreTrainedPolicy(nn.Module, HubMixin, abc.ABC):
    ...
    def _load_as_safetensor(cls, model: T, model_file: str, map_location: str, strict) -> T:
        ...
        # Load the model with appropriate kwargs. Older LabVLA checkpoints saved
        # both sides of Qwen's tied lm-head/input-embedding alias, while current
        # model state_dicts expose only the canonical lm_head key. Preserve
        # strict loading for all real mismatches, but tolerate that legacy alias.
        allowed_unexpected = {"model.vlm.model.language_model.embed_tokens.weight"}
        try:
            ...
            relaxed_kwargs["strict"] = False
            missing_keys, unexpected_keys = load_model_as_safetensor(model, model_file, **relaxed_kwargs)
            residual_missing = set(missing_keys)
            residual_unexpected = set(unexpected_keys) - allowed_unexpected
            if residual_missing or residual_unexpected:
                raise
            logging.warning(
                "Strict safetensor load accepted legacy tied embedding alias: %s",
                sorted(allowed_unexpected & set(unexpected_keys)),
            )
