...
def convert_json_array_to_jsonl(
    ...
    *,
    ...
    if output_jsonl.exists() and offsets_path.exists() and not force:
        ...
        # Reindexing only re-derives offsets from the existing JSONL; it proves
        # nothing about the JSONL still matching the source, so the source
        # fingerprint must NOT be re-stamped here (only a --force full
        # regeneration replays the source and may stamp).
        print(f"[index] rebuilt {offsets_path}; records={len(offsets)} "
              f"(source fingerprint left untouched — M69)")
        return
    if output_jsonl.exists() and not force:
        offsets = build_offsets_from_jsonl(output_jsonl)
        write_offsets(offsets_path, offsets)
