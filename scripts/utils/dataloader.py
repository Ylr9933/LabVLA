...
# Exception classes that indicate a DETERMINISTIC schema/data contract
# violation (ragged widths, overwide rows, missing virtual-source column,
# delta-mask mismatch). SkipBadSamplesDataset re-raises these instead of
# resampling: retrying cannot fix them and skipping silently reweights the
# data. pyarrow decode errors subclass ValueError (ArrowInvalid), so they are
# explicitly exempted — corrupt-shard IO stays retryable.
_CONTRACT_ERROR_TYPES = (KeyError, ValueError, TypeError, AssertionError)
try:
    import pyarrow as _pa
    _ARROW_ERROR_TYPES = (_pa.lib.ArrowException,)
except Exception:  # pragma: no cover - pyarrow always present in the env
    _ARROW_ERROR_TYPES = ()


def _skip_contract_errors() -> bool:
    """LABVLA_DATA_SKIP_CONTRACT_ERRORS=1 restores the legacy swallow-everything
    skip behavior (see src/utils/env_flags.py)."""
    return os.environ.get("LABVLA_DATA_SKIP_CONTRACT_ERRORS") == "1"
...
class SkipBadSamplesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # (see _CONTRACT_ERROR_TYPES below for the skip/raise split)
        base_idx = int(idx)
        ...
        for attempt in range(self.max_attempts):
            ...
            try:
                ...
                if isinstance(exc, _CONTRACT_ERROR_TYPES) and not isinstance(
                    exc, _ARROW_ERROR_TYPES
                ) and not _skip_contract_errors():
                    # Deterministic schema-contract violations (ragged widths,
                    # overwide rows, missing virtual-source columns, delta-mask
                    # mismatches) resample forever and silently REWEIGHT the
                    # data if skipped; only IO/decode flakiness is retryable.
                    # LABVLA_DATA_SKIP_CONTRACT_ERRORS=1 restores old behavior.
                    raise
                last_err = exc
