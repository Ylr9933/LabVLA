...
# ("width", "open_fraction") is INTENTIONALLY absent from this set: both
# semantics are bimodal endpoint signals, and per-repo q01/q99 normalization
# maps each repo's {closed, open} endpoints to {-1, +1} equivalently, so a
# width+open_fraction MSE mix (e.g. ki_posttrain's oxe-auge + width-gripper repos) is
# scale-consistent. "position"/"velocity" stay incompatible with everything
# else: they are not endpoint-bimodal, so q01/q99 aligns scale but NOT meaning.
...
def _resolve_gripper_semantic(schema):
    ...
    if not sid:
        return None
    try:
        from src.schema.registry import registered_names, resolve as _resolve_schema
...
def _check_gripper_semantic_compat(repo_ids, schemas, *, ignored_repo_ids=None) -> None:
    ...
    for rid, sch in zip(repo_ids, schemas):
        ...
        if sem is None:
            if rid not in _GRIPPER_SEM_NONE_WARNED:
                _GRIPPER_SEM_NONE_WARNED.add(rid)
                logging.getLogger(__name__).warning(
                    "[gripper-semantic] repo %r resolves to gripper_semantic="
                    "None — excluded from the cross-repo compatibility check "
                    "(mixing it with velocity/width repos is unchecked). "
                    "Declare gripper_semantic in its schema to include it.",
                    rid,
                )
            continue
