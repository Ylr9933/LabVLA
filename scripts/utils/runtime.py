...
def resume_start_batch_index(
    step: int, epoch: int, total_batches: int, num_processes: int
) -> int:
    """Inner-batch offset for resuming a sharded batch sampler.

    The training loop's saved ``step`` counts PER-RANK micro-batches. After
    ``accelerator.prepare``, the inner batch sampler is wrapped by accelerate's
    ``BatchSamplerShard`` (``split_batches=False``), so ONE loop step consumes
    ``num_processes`` inner batches GLOBALLY. The old computation
    ``step % total_batches`` under-skipped by a factor of ``num_processes``,
    silently replaying ~``step*(W-1)`` already-trained batches on every
    multi-GPU full resume (correct only at W=1, which masked the bug).

    Each epoch contributes ``ceil(total_batches / W)`` steps per rank
    (``even_batches`` pads the tail by cycling from the start), so the
    within-epoch step count is derived from the saved ``epoch`` first, then
    converted to inner batches. Tail-padding cycles can make the product
    overshoot ``total_batches`` by up to ``W - 1``; the modulo wrap is an
    off-by-<W approximation once per epoch boundary, versus the old factor-W
    under-skip everywhere.
    """
    w = max(1, int(num_processes))
    total = max(1, int(total_batches))
    per_rank_epoch_len = math.ceil(total / w)
    steps_into_epoch = max(0, int(step) - int(epoch) * per_rank_epoch_len)
    return (steps_into_epoch * w) % total
...
class BackgroundBatchBuffer:
    ...
    def __init__(
        ...
        # Seed the epoch counter BEFORE the producer thread starts; a
        # post-construction `buffer._epoch = N` would race a warmup
        # StopIteration on small datasets (advance lost on overwrite).
        self._epoch = int(start_epoch)
        # `_epoch` is the PRODUCER cursor — at an epoch boundary the producer
        # may already be prefetching epoch N+1 while training still consumes
        # queued epoch-N batches. Checkpoints must record the CONSUMER position,
        # so each queued batch is tagged with its production epoch and `epoch`
        # reports the last dequeued tag.
        self._consumer_epoch = int(start_epoch)
    ...
    def _fill_loop(self) -> None:
        try:
            ...
            self._error = e
            try:
                self._queue.put((self._SENTINEL, None, None), timeout=5.0)
            except queue.Full:
                pass
    ...
    def get(self, timeout: int = 1800):
        ...
        try:
            batch, event, _src_epoch = self._queue.get(timeout=timeout)
        except queue.Empty:
            if self._error is not None:
                raise RuntimeError(
                    f"BackgroundBatchBuffer background thread terminated with error: {self._error}"
        ...
        if _src_epoch is not None:
            self._consumer_epoch = int(_src_epoch)
    ...
    @property
    def epoch(self) -> int:
        # Consumer-side epoch: the epoch of the last batch handed to the
        # training loop — the value that belongs in a checkpoint.
        return self._consumer_epoch
...
def _set_epoch_recursive(ds, epoch: int) -> int:
    ...
    def _walk(obj) -> None:
        ...
        # Include "image_transforms" so ImageTransforms.set_epoch is actually
        # reached (it hangs off the adapter, not the dataset chain); otherwise
        # augmentation seeds stay frozen at epoch 0.
        for attr in (
            "_datasets", "datasets", "dataset", "adapter", "_adapter",
            "image_transforms",
        ):
