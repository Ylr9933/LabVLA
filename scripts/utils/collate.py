        ...
        for key in all_keys:
            ...
            if isinstance(value, torch.Tensor):
                if key.endswith("_is_pad"):
                    # A zero-filled pad mask means "every timestep is REAL
                    # supervision" — the exact semantic inversion of a missing
                    # mask. If the paired value key is also a batch-level
                    # placeholder, mark the whole sample as padding (no
                    # supervision); a sample that carries a real value WITHOUT
                    # its mask is a transform-contract violation and must fail
                    # loud.
                    _val_key = key[: -len("_is_pad")]
                    if (_val_key in sample) and (_val_key not in _filled) and (_val_key in all_keys):
                        raise ValueError(
                            f"collate: sample carries {_val_key!r} but not its "
                            f"pad mask {key!r} — refusing to fabricate a "
                            f"zero (all-valid) mask for real supervision "
                            f"(H27). Fix the producing transform."
                        )
                    sample[key] = torch.ones_like(value)
                else:
