...
def parse_args():
    ...
    parser.add_argument("--max_consecutive_nan_skips", type=int, default=25,
                        help="Abort policy threshold for consecutive NaN/Inf-loss "
                             "skips (M23). 0 disables the streak abort entirely "
                             "(NOT recommended); default 25.")
    ...
    # Output
    # Default inside this checkout (scripts/utils/ -> repo root), never a
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "checkpoint's accelerate_state/ (optimizer/scheduler/RNG) + training_state.pt (metadata/step). Use this to resume an "
    ...
    # ModelEMA (src/optim/ema.py) requires decay in the OPEN interval (0,1);
    # 1.0 passes the inclusive `_vf > _hi` check above but raises at ModelEMA
    # construction AFTER accelerator.prepare. Fail at argparse instead. (0.0 is
    # allowed: it disables EMA in train.py.)
    _ema = getattr(args, "ema_decay", None)
    if _ema is not None and float(_ema) >= 1.0:
        parser.error(
            f"--ema_decay must be < 1.0 (got {_ema}). decay=1.0 freezes the EMA "
            f"(never tracks weights) and ModelEMA rejects it."
        )
