...
def build_dataset(args):
    ...
    from src.transforms.core import hydrate_all, compose
    from src.dataset.utils import load_json, cast_stats_to_numpy
    from src.dataset.transforms import ImageTransforms
    from src.policies.LabVLA.configuration_labvla import LabVLADatasetConfig
    ...
    # VQA placeholder tensors are generated from the ROBOINTER_BUDGET singleton,
    # NOT from the runtime config — a non-default chunk/state/action dim would
    # silently produce mis-shaped placeholders. Guard until the budget is
    # properly plumbed.
    def _is_probably_vqa_repo(_rid: str) -> bool:
        _root = Path(data_root) / _rid
        return (_root / "meta" / "vqa_manifest.json").exists()

    if any(_is_probably_vqa_repo(r) for r in repo_ids):
        from src.dataset.adapters.robointer_token_budget import ROBOINTER_BUDGET as _RB
        _mismatch = []
        for _name, _args_v, _bud_v in (
            ("chunk_size", getattr(args, "chunk_size", None), _RB.chunk_size),
            ("max_state_dim", getattr(args, "max_state_dim", None), _RB.max_state_dim),
            ("max_action_dim", getattr(args, "max_action_dim", None), _RB.max_action_dim),
        ):
            if _args_v is not None and int(_args_v) != int(_bud_v):
                _mismatch.append(f"{_name}: args={_args_v} budget={_bud_v}")
        if _mismatch:
            raise ValueError(
                "VQA repos are in the mixture but ROBOINTER_BUDGET geometry "
                "does not match the runtime config (M83): "
                + "; ".join(_mismatch)
                + ". Update src/dataset/adapters/robointer_token_budget.py "
                "to match, or drop the VQA repos."
            )
    ...
    if schema_spec_arg:
        from src.schema import resolve
        if "=" in schema_spec_arg:
            for entry in schema_spec_arg.split(","):
                ...
                repo = repo.strip()
                if repo in per_repo_schema:
                    raise ValueError(
                        f"--dataset_schema map repeats repo key {repo!r} "
                        f"(M29: last-write-wins would silently mask one spec)."
                    )
                per_repo_schema[repo] = resolve(spec.strip())
        else:
            resolved = resolve(schema_spec_arg.strip())
            for rid in repo_ids:
                per_repo_schema[rid] = resolved
        # Every explicitly-mapped repo key must belong to this launch — a typo'd
        # key would silently fall back to manifest/auto-infer.
        _unknown = sorted(set(per_repo_schema) - set(repo_ids))
        if _unknown:
            raise ValueError(
                f"--dataset_schema maps repo key(s) {_unknown} that are not in "
                f"--repo_ids {sorted(repo_ids)} (M29): the override would "
                f"silently miss. Fix the key(s)."
            )
        for rid in repo_ids:
    ...
    if external_stats_map:
        # A typo'd repo key would silently fall back to the repo's own (possibly
        # stale) stats — for OXE-class overrides that reopens the exact
        # wrong-normalization path the override exists to close.
        _unknown_stats_keys = sorted(set(external_stats_map) - set(repo_ids))
        if _unknown_stats_keys:
            raise ValueError(
                f"--external_stats_map contains repo key(s) "
                f"{_unknown_stats_keys} not present in --repo_ids "
                f"{sorted(repo_ids)} (M48): the override would silently miss."
            )
        logging.info(
    ...
    if getattr(args, "image_augmentation", True):
        from src.dataset.transforms import ImageTransformsConfig, ImageTransformConfig
    ...
    ds_cfg = LabVLADatasetConfig(
        ...
        fast_tokenizer_path=getattr(args, "fast_tokenizer_path", "physical-intelligence/fast"),
    ...
    for repo_id in repo_ids:
        ...
        if effective_external_stats:
            ...
            # The external file carries raw action/action_abs entries, which
            # just overwrote the adapter's patch_stats_for_next_frame_actions
            # output (gripper dim rebased from velocity to position stats).
            # Re-apply the patch on the merged dict so NEXT_FRAME schemas +
        ...
        if repo_id in action_supervision_skip_repos:
            from src.transforms.fast_action import DropFastActionSupervisionTransformFn
    ...
    from src.dataset.pi0_mixture import PI0MixtureDataset
