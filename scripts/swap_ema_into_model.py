...
def main() -> None:
    ...
    matched = 0
    matched_numel = 0
    model_total_numel = sum(int(t.numel()) for t in model_state.values())
    missing = []
    skipped_shape = []
    for name, live_tensor in model_state.items():
        ...
        if ema_tensor is None and name.endswith("lm_head.weight"):
            # Tied weights: EMA tracked named_parameters(remove_duplicate=True),
            # which keeps only the DFS-first embed_tokens name, while
            # _clone_shared_tensors_for_safetensors keeps the *lm_head* name in
            # model.safetensors — without this alias the largest tensor in the
            # model (~389M params) silently stayed live in the "EMA" export.
            prefix = name[: -len("lm_head.weight")]
            for cand in (
                prefix + "model.language_model.embed_tokens.weight",
                prefix + "language_model.embed_tokens.weight",
                prefix + "model.embed_tokens.weight",
            ):
                ema_tensor = ema_state.get(cand)
                if ema_tensor is None and cand.startswith("model."):
                    ema_tensor = ema_state.get(cand[len("model."):])
                if ema_tensor is not None:
                    print(f"  tied-weight alias: {name} <- {cand}")
                    break
        ...
        matched += 1
        matched_numel += int(live_tensor.numel())
    ...
    numel_frac = (matched_numel / model_total_numel) if model_total_numel else 0.0
    print(f"EMA coverage: {coverage:.4f} ({matched}/{ema_total} EMA tensors applied; "
          f"{numel_frac:.4f} of model elements swapped)")
    problems = []
    if ema_total == 0:
    ...
    _tmp = model_path.with_name(model_path.name + ".tmp-swap")
    save_file(model_state, str(_tmp))
    os.replace(_tmp, model_path)
    print("done")
