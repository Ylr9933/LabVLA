...
# Import the public checkpoint API (bound to the historical local names so the
# loop body stays diff-free; the cross-module contract is the public surface
# declared in scripts/utils/checkpoint.py.__all__).
from src.utils.env_flags import validate_environment as _validate_env_flags
from scripts.utils.checkpoint import (
    QWEN_BUNDLED_ASSET_NAMES,
    accelerate_state_dir as _accelerate_state_dir,
    checkpoint_incomplete_marker as _checkpoint_incomplete_marker,
    clear_checkpoint_incomplete_marker as _clear_checkpoint_incomplete_marker,
    is_recoverable_checkpoint_save_error as _is_recoverable_checkpoint_save_error,
    load_training_state as _load_training_state,
    rotate_checkpoints as _rotate_checkpoints,
    torch_load_with_retry as _torch_load_with_retry,
    torch_save_with_retry as _torch_save_with_retry,
    validate_full_resume_contract as _validate_full_resume_contract,
    write_checkpoint_incomplete_marker as _write_checkpoint_incomplete_marker,
    load_checkpoint,
    save_checkpoint,
)
...
from scripts.utils.runtime import (
    ...
    format_time,
    resume_start_batch_index,
    set_seed,
    setup_logging,
)
from src.utils.storage_retry import run_with_storage_retry
...
# --------------------------------------------------------------------------- #
# train() stage helpers. Every body below is a VERBATIM line-range
# move out of the former 1515-line train(); only the def/return plumbing and
# explicit parameters are new. train() is the thin orchestrator.
# --------------------------------------------------------------------------- #
...
def _setup_accelerator_and_signals(args):
    """Stage A (verbatim move): device pinning, Accelerator + DS grad-clip
    injection, new_group timeout patch, signal handlers, banner, seeding.

    Returns (accelerator, is_main, device, graceful_dict, restore_new_group):
    ``restore_new_group`` undoes the new_group monkey-patch immediately at the
    end of train() (the atexit hook stays as the crash-path fallback) —
    replaces the old `"_tdist" in dir()` tail check with an explicit handle.
    """
    def _restore_new_group_noop():
        return None
    _restore_new_group_now = _restore_new_group_noop
    ...
    try:
        ...
        _restore_new_group_now = _restore_new_group
    ...
    return accelerator, is_main, device, _graceful, _restore_new_group_now


def _stabilize_vlm_assets(bundle_dir, args):
    """Copy the small bundled VLM assets OUT of the resume checkpoint into a
    stable per-source dir under output_dir.

    GPU-resume smoke 2026-06-11 follow-up: after a self-contained resume,
    policy.config.vlm_pretrained_path points INTO an old checkpoint; every
    later save bundles from there, and once _rotate_checkpoints deletes that
    old checkpoint the bundling silently fails — later checkpoints stop being
    self-contained and the NEXT resume dies on the full-resume contract.
    A stable copy outside the rotation-managed checkpoint-* dirs breaks the
    hazard chain. Rank-race-safe (tmp + os.replace per file; identical bytes).
    Falls back to the in-checkpoint dir if the copy fails (legacy behavior).
    """
    import hashlib
    ...
    try:
        # The digest is the CONTENT hash of vlm_config.json, making the stable
        # dir self-verifying — the full-resume exemption recomputes this hash
        # from the dir's own vlm_config.json and refuses paths whose name does
        # not match.
        digest = hashlib.sha256(
            (bundle_dir / "vlm_config.json").read_bytes()
        ).hexdigest()[:16]
        stable = Path(args.output_dir) / "_vlm_assets" / digest
        marker = stable / "vlm_config.json"
        if marker.is_file():
            # Reuse only when the existing dir still hashes to its own name
            # (a stale/foreign dir must be refreshed, not silently served).
            _existing = hashlib.sha256(marker.read_bytes()).hexdigest()[:16]
            if _existing != digest:
                import shutil as _sh
                _sh.rmtree(stable, ignore_errors=True)
        if not marker.is_file():
            stable.mkdir(parents=True, exist_ok=True)
            names = [n for n in QWEN_BUNDLED_ASSET_NAMES if n != "vlm_config.json"]
            names.append("vlm_config.json")  # completion marker written last
            for name in names:
                src = bundle_dir / name
                if not src.is_file():
                    continue  # optional assets may be absent in older bundles
                tmp = stable / f".{name}.tmp.{uuid.uuid4().hex}"
                _shutil.copyfile(src, tmp)
                os.replace(tmp, stable / name)
        if (stable / "vlm_config.json").is_file():
            return stable
    except OSError as e:
        logging.warning(
            "[self-contained] could not stabilize VLM assets under %s (%s); "
            "falling back to the in-checkpoint bundle (rotation may delete "
            "the bundle source mid-run).", args.output_dir, e,
        )
    return bundle_dir


def _resolve_self_contained_resume(args):
    """Stage B (verbatim move): point vlm_pretrained_path at a resume ckpt's
    bundled VLM assets when present (mutates args in place)."""
    # Self-contained checkpoint resume: if --resume bundles vlm_assets/ (Qwen3-VL
    # config + tokenizer + processor, no weights), build the VLM weightlessly from the
    # bundle — the resume ckpt's model.safetensors fills the weights via load_checkpoint
    # — instead of needing an external Qwen base. build_config reads args.vlm_pretrained_path.
    if getattr(args, "resume", None):
        for _cand_dir in (Path(args.resume) / "pretrained_model", Path(args.resume)):
            if (_cand_dir / "vlm_config.json").is_file():
                logging.info(
                    f"[self-contained] resume ckpt bundles VLM assets in {_cand_dir}; "
                    f"using it as vlm_pretrained_path (was {args.vlm_pretrained_path!r})"
                )
                args.vlm_pretrained_path = str(_stabilize_vlm_assets(_cand_dir, args))
                if args.vlm_pretrained_path != str(_cand_dir):
                    logging.info(
                        "[self-contained] VLM assets stabilized to %s "
                        "(rotation-safe bundle source)", args.vlm_pretrained_path
                    )
                break


def _build_dataset_and_schemas(args, config, is_main):
    """Stage C (verbatim move): dataset build, schema-derived config fixups,
    multi-repo guards, annotation/liger probes, norm-stats extraction."""
    ...
    if _active_schema is not None:
        from src.dataset.utils import PolicyFeature, FeatureType
    ...
    if data_stats:
        from src.dataset.utils import serialize_dict
        if len(data_stats) == 1:
    ...
    return (dataset, data_stats, data_schemas, _active_schema,
            _has_annotation_losses, _active_norm_stats)


def _precheck_resume_contract(args, is_main):
    """Stage D (verbatim move): full-resume contract validation before any
    heavy construction."""
    ...
    if full_resume_requested:
        ...
        if not pre_resume_train_state and resume_uses_accelerate_state:
            # Without this, full resume silently restarts at step 0 while
            # accelerator.load_state still restores optimizer/scheduler.
            raise RuntimeError(
                f"Checkpoint {args.resume} has accelerate_state/ but no "
                "readable training_state.pt — refusing a full resume that "
                "would silently replay from step 0 with a restored optimizer. "
                "Pick an intact checkpoint or use --load_weights_only true."
            )
        if pre_resume_train_state:
            _validate_full_resume_contract(pre_resume_train_state.get("args", {}) or {}, args)
            # training_state.pt and the sibling accelerate_state/ must come from
            # the SAME save — reject mixed/partially-synced dirs before any
            # optimizer/RNG state is restored.
            _saved_ckpt_id = pre_resume_train_state.get("checkpoint_id")
            if _saved_ckpt_id and resume_uses_accelerate_state:
                _id_file = resume_accelerate_state_dir / "labvla_checkpoint_id.json"
                if _id_file.is_file():
                    try:
                        _disk_id = json.loads(_id_file.read_text()).get("checkpoint_id")
                    except (OSError, ValueError) as _e:
                        raise RuntimeError(
                            f"Checkpoint {args.resume}: cannot read "
                            f"accelerate_state/labvla_checkpoint_id.json ({_e}) — "
                            "refusing full resume with unverifiable distributed-state "
                            "identity."
                        )
                    if _disk_id != _saved_ckpt_id:
                        raise RuntimeError(
                            f"Checkpoint {args.resume}: training_state.pt "
                            f"(checkpoint_id={_saved_ckpt_id}) and accelerate_state/ "
                            f"(checkpoint_id={_disk_id}) come from DIFFERENT saves — "
                            "the dir was hand-assembled or partially synced. "
                            "Refusing full resume: optimizer/scheduler/RNG would be "
                            "restored from a mismatched state."
                        )
                else:
                    logging.warning(
                        "[H21] %s has no accelerate_state/labvla_checkpoint_id.json "
                        "(pre-binding checkpoint); sibling identity cannot be "
                        "verified for this resume.", args.resume,
                    )
    ...
    return (full_resume_requested, resume_accelerate_state_dir,
            resume_uses_accelerate_state, pre_resume_train_state,
            pre_resume_start_step, pre_resume_start_epoch)


def _build_dataloader(args, dataset, accelerator, is_main,
                      full_resume_requested, pre_resume_start_step,
                      pre_resume_start_epoch):
    """Stage E (verbatim move): worker seeding, collator, batch sampler
    (homogeneous-mixture / deterministic / plain), DS auto batch sizes."""
    ...
    if getattr(args, "homogeneous_mixture_batches", False):
        from src.dataset.pi0_mixture import PI0MixtureBatchSampler, PI0MixtureDataset

        if isinstance(dataset, PI0MixtureDataset):
            total_batches = max(1, len(dataset) // int(args.batch_size))
            # The saved `step` counts per-rank micro-batches while the inner
            # sampler indexes GLOBAL batches (accelerate BatchSamplerShard
            # consumes num_processes inner batches per loop step). A
            # `step % total_batches` would under-skip by a factor of
            # num_processes, silently replaying already-trained data after every
            # multi-GPU full resume.
            start_batch_index = (
                resume_start_batch_index(
                    pre_resume_start_step,
                    pre_resume_start_epoch,
                    total_batches,
        ...
        total_batches = max(1, len(dataset) // int(args.batch_size))
        # Same global-vs-per-rank step conversion as the homogeneous-mixture
        # path above.
        start_batch_index = (
            resume_start_batch_index(
                pre_resume_start_step,
...
def _prepare_output_dir(args, accelerator, is_main):
    """Stage F (verbatim move): job_name default, output dir, train.log."""
...
def _build_policy_and_optimizer(args, config, is_main):
    """Stage G (verbatim move): policy construction + GA-aware optimizer/
    scheduler horizon."""
    if is_main:
    ...
    if _ga > 1 and is_main:
        if args.steps % _ga != 0:
            # The trailing micro-batches consume data/compute but never form an
            # optimizer update — their gradients are silently dropped. Refuse
            # instead of warning.
            raise ValueError(
                f"[T1/M41] --steps={args.steps} is not a multiple of "
                f"gradient_accumulation_steps={_ga}: the final "
                f"{args.steps % _ga} micro-batch(es) would never reach a sync "
                f"boundary and their gradients would be dropped. Round "
                f"--steps to a multiple of the accumulation factor."
            )
        logging.info(
...
def _metrics_file_logger():
    """A logger that carries the run's train.log file handler ONLY (no
    console propagation), so periodic metrics land in the file without
    double-printing next to the tqdm bar."""
    global _METRICS_LOGGER
    if _METRICS_LOGGER is None:
        lg = logging.getLogger("labvla.train.metrics")
        lg.propagate = False
...
def _check_warmstart_not_incomplete(args):
    """Weights-only warmstart must honor the .INCOMPLETE marker that full
    resume honors, so a partially-saved checkpoint is not consumed as
    legitimate pretrained weights."""
    if args.resume and getattr(args, "load_weights_only", False):
        _marker = _checkpoint_incomplete_marker(args.resume)
        if _marker.exists():
            raise RuntimeError(
                f"Checkpoint {args.resume} carries {_marker.name} (a save "
                "failed mid-write) — refusing weights-only warmstart from a "
                "partial checkpoint. Pick an intact checkpoint."
            )


def _load_weights_pre_prepare(args, is_main, policy,
                              resume_uses_accelerate_state,
                              resume_accelerate_state_dir):
    """Stage H (verbatim move): the `if args.resume:` body that loads model
    weights BEFORE accelerator.prepare (see call site for rationale)."""
...
def _restore_state_post_prepare(args, accelerator, optimizer, scheduler,
                                dataset, is_main,
                                resume_uses_accelerate_state,
                                resume_accelerate_state_dir,
                                pre_resume_train_state):
    """Stage I (verbatim move): post-prepare full-resume restore (distributed
    state or legacy training_state.pt). Returns (start_step, start_epoch)."""
    ...
    if resume_uses_accelerate_state:
        ...
        if "optimizer_state_dict" in train_state:
            try:
                ...
                if is_main:
                    logging.info(
                        "Restored optimizer state from legacy training_state.pt "
                        "(rank0 shard applied on ALL ranks - debugging only, "
                        "gated by --allow_legacy_partial_resume)"
                    )
            except Exception as e:
                if is_main:
                    logging.warning(
...
def _recast_ki_after_prepare(policy, is_main):
...
def _setup_ema(args, policy, is_main):
    ...
    if ema_decay_arg > 0.0:
        ...
        # Full resume only: warmstart (--load_weights_only) means "weights
        # only, step 0" — restoring the old EMA buffer/decay there silently
        # continues the previous run's average.
        if args.resume and not getattr(args, "load_weights_only", False):
            ...
            if _ema_path.exists():
                try:
                    ...
                    # load_state_dict adopts the saved decay; the CLI value
                    # wins (full-resume contract pins them equal anyway).
                    ema.decay = ema_decay_arg
                    if is_main:
                        logging.info(
                    ...
                    # Silently restarting the EMA average mid-run breaks every
                    # EMA-based eval/export downstream — fail loud on a full
                    # resume.
                    raise RuntimeError(
                        f"Full resume: EMA state at {_ema_path} exists but "
                        f"could not be restored ({e}). Refusing to silently "
                        "restart the EMA average; fix/regenerate the "
                        "checkpoint or warmstart with --load_weights_only."
                    ) from e
            else:
                # ema_decay is in the full-resume contract, so the prior run
                # MUST have saved an EMA state — absence means a damaged or
                # hand-assembled checkpoint.
                raise RuntimeError(
                    f"Full resume with ema_decay>0 but {_ema_path} is missing "
                    "— the source run saved EMA state at every checkpoint "
                    "(contract pins ema_decay). Refusing to silently restart "
                    "the EMA average."
                )
...
def _make_nan_streak_guard(accelerator, policy, is_main, nan_skip_abort):
    """Factory for the NaN-streak abort guard (verbatim move of the two
    closures; the streak counter itself stays a train() local because the
    loop mutates it)."""
    ...
    def _abort_or_warn_on_nan_streak(streak: int, force: bool = False):
        ...
        # nan_skip_abort<=0 means "disable the streak abort" (per the CLI
        # help). Without the `<=0` guard, `streak < 0` is always
        # False and the value 0 would INVERT to the most aggressive behavior.
        # force=True (DeepSpeed grad-NaN check) still runs regardless.
        if not force and (nan_skip_abort <= 0 or streak < nan_skip_abort):
            return
    ...
    return _abort_or_warn_on_nan_streak


def _save_training_checkpoint(accelerator, args, output_dir, current_step,
                              policy, optimizer, scheduler, ema, dataset,
                              batch_buffer, nominal_epoch, active_schema,
                              active_norm_stats, data_schemas, is_main):
    """Verbatim move of the in-loop checkpoint block (the loop's deepest
    nesting): distributed state, rank0 artifacts, EMA state, `last` symlink,
    rotation, video-fallback summary."""
    accelerator.wait_for_everyone()
    ...
    if is_main:
        try:
            ...
            if active_schema is None and active_norm_stats is not None:
                raise RuntimeError(
            ...
            save_checkpoint(
                ...
                # Topology identity for resume-time verification.
            ...
            # train_utils lives outside src/utils because it is
            # orchestration-level code; living there dragged
            # config/dataset/optim/policies into the base package.
            from scripts.utils.train_utils import update_last_checkpoint
...
def train(args):
    # train() is a thin orchestrator over the stage helpers above; every stage
    # body is a verbatim move of its former inline block.
    (accelerator, is_main, device, _graceful,
     _restore_new_group_now) = _setup_accelerator_and_signals(args)

    if is_main:
        # Wire the env-flag typo guard (env_flags.validate_environment()) into
        # the entrypoint.
        _validate_env_flags()

    # A full resume without an explicit --job_name must NOT mint a NEW
    # timestamped run dir (that would silently fork checkpoints/logs away from
    # the original run). Default back into the resume checkpoint's own run
    # directory instead.
    if (args.resume and not getattr(args, "load_weights_only", False)
            and getattr(args, "job_name", None) is None):
        _resume_run_dir = Path(args.resume).resolve().parent
        args.output_dir = str(_resume_run_dir.parent)
        args.job_name = _resume_run_dir.name
        logging.info(
            "[M67] full resume without --job_name: continuing IN the resume "
            "run dir %s (pass an explicit --job_name to fork deliberately).",
            _resume_run_dir,
        )

    _check_warmstart_not_incomplete(args)
    _resolve_self_contained_resume(args)
    config = build_config(args)

    if is_main:
        logging.info("Building dataset...")
    (dataset, data_stats, data_schemas, _active_schema,
     _has_annotation_losses, _active_norm_stats) = _build_dataset_and_schemas(
        args, config, is_main
    )

    (full_resume_requested, resume_accelerate_state_dir,
     resume_uses_accelerate_state, pre_resume_train_state,
     pre_resume_start_step, pre_resume_start_epoch) = _precheck_resume_contract(
        args, is_main
    )
    # Full resume across a different world size / mixed precision cannot restore
    # the ZeRO shards correctly — verify the recorded topology identity before
    # any state is loaded.
    _saved_dist_id = (pre_resume_train_state or {}).get("distributed_identity") or {}
    if full_resume_requested and _saved_dist_id:
        _cur_np = int(accelerator.num_processes)
        _saved_np = int(_saved_dist_id.get("num_processes", _cur_np))
        _cur_mp = str(getattr(accelerator, "mixed_precision", ""))
        _saved_mp = str(_saved_dist_id.get("mixed_precision", _cur_mp))
        if _saved_np != _cur_np or _saved_mp != _cur_mp:
            raise RuntimeError(
                f"Full resume topology mismatch (M78): checkpoint was saved "
                f"with num_processes={_saved_np}, mixed_precision="
                f"{_saved_mp!r} but the current launch is num_processes="
                f"{_cur_np}, mixed_precision={_cur_mp!r}. ZeRO-sharded "
                f"optimizer state cannot be restored across this change; "
                f"relaunch with the original topology or warmstart with "
                f"--load_weights_only true."
            )

    dataloader = _build_dataloader(
        args, dataset, accelerator, is_main,
        full_resume_requested, pre_resume_start_step, pre_resume_start_epoch,
    )

    output_dir = _prepare_output_dir(args, accelerator, is_main)

    policy, optimizer, scheduler = _build_policy_and_optimizer(
        args, config, is_main
    )
    ...
    if args.resume:
        _load_weights_pre_prepare(
            args, is_main, policy,
            resume_uses_accelerate_state, resume_accelerate_state_dir,
        )
    ...
    if args.resume and not getattr(args, "load_weights_only", False):
        start_step, start_epoch = _restore_state_post_prepare(
            args, accelerator, optimizer, scheduler, dataset, is_main,
            resume_uses_accelerate_state, resume_accelerate_state_dir,
            pre_resume_train_state,
        )

    # Launchers DISCOVER repo lists and weights locally per node — divergent
    # mounts/sort orders would train different mixtures per rank. Verify parity
    # before stepping.
    if accelerator.num_processes > 1:
        import hashlib as _hl
        _mix_sig = _hl.sha256(
            f"{getattr(args, 'repo_ids', '')}|{getattr(args, 'dataset_weights', '')}".encode()
        ).hexdigest()
        _sigs = [None] * accelerator.num_processes
        torch.distributed.all_gather_object(_sigs, _mix_sig)
        if len(set(_sigs)) != 1:
            raise RuntimeError(
                f"[M111] repo_ids/dataset_weights differ across ranks "
                f"(signatures: {sorted(set(_sigs))}). Each node discovered a "
                f"different data mixture — fix the data mounts or pin an "
                f"explicit RepoIds list before training."
            )
        if is_main:
            logging.info("[M111] cross-rank mixture parity OK (%d ranks)",
                         accelerator.num_processes)

    _recast_ki_after_prepare(policy, is_main)

    ema = _setup_ema(args, policy, is_main)

    wandb_logger = _setup_wandb(args, is_main)
    ...
    if use_prefetch_buffer:
        batch_buffer = BackgroundBatchBuffer(
            ...
            # Seed the producer-thread counter pre-start so resume continues
            # from the checkpointed epoch (post-construction assignment raced
            # the warmup fill loop).
    ...
    _nan_skip_streak = 0
    # The CLI flag exists, and 0 means "disabled" instead of being silently
    # folded back to 25.
    _NAN_SKIP_ABORT = int(getattr(args, "max_consecutive_nan_skips", 25))
    if (getattr(accelerator.state, "deepspeed_plugin", None) is not None
            and int(getattr(args, "gradient_accumulation_steps", 1) or 1) > 1
            and is_main):
        logging.warning(
            "[nan-guard] DeepSpeed + gradient_accumulation_steps>1: a loss-NaN "
            "skip advances accelerate's accumulate() counter but not the DS "
            "engine micro-step, so sync_gradients and the DS optimizer boundary "
            "desynchronize by one micro-step per skip (scheduler/EMA advance "
            "off-boundary afterwards). Known limitation — production launches "
            "use GA=1 with DS."
        )
    ...
    _abort_or_warn_on_nan_streak = _make_nan_streak_guard(
        accelerator, policy, is_main, _NAN_SKIP_ABORT
    )
    ...
    for step in range(start_step, args.steps):
        ...
        # Weight by valid (non-padded) action timesteps so mixed batches with
        # very different padding don't contribute equally to the window.
        _n_valid_action = 1
        _aip = batch.get("action_is_pad") if isinstance(batch, dict) else None
        if _aip is not None:
            try:
                _n_valid_action = max(1, int((~_aip.bool()).sum().item()))
            except Exception:  # noqa: BLE001 — meter weighting is best-effort
                _n_valid_action = 1
        loss_action_meter.update(output_dict["loss_action"], n=_n_valid_action)
        ...
        if args.log_freq > 0 and current_step % args.log_freq == 0 and is_main:
            ...
            if pbar is not None:
                tqdm.write(log_msg)
                # Periodic metrics must also reach train.log — tqdm.write only
                # goes to the console stream. (The no-pbar branch below already
                # lands in the file via root logging.)
                _metrics_file_logger().info(log_msg)
            else:
            ...
            if wandb_logger:
                wandb_payload = {
                    ...
                    # How many sync steps the grad_norm window covers.
                    "grad_norm_window_n": grad_norm_meter.count,
                    # Optimizer-step counter (micro-steps / GA). current_step is
                    # already 1-indexed (step+1); the true optimizer-step count
                    # is current_step // GA (no extra +1).
                    "optimizer_step": current_step // max(
                        1, int(args.gradient_accumulation_steps)),
                    "lr": optimizer.param_groups[0]["lr"],
                    # Differential LR — one curve per named param group.
                    **{f"lr/{g.get('name', f'group{i}')}": g["lr"]
                ...
                if pdl is not None and pdd is not None:
                    try:
                        ...
                        for i in range(min(int(pdd), len(pdl_host))):
                            # These are LAST-BATCH instantaneous values
                            # while loss/loss_action above are window means —
                            # name them so the units can't be conflated.
                            wandb_payload[f"loss_action_dim{i}_lastbatch"] = float(pdl_host[i])
                    except Exception as _e:
        ...
        if current_step % args.save_freq == 0 or current_step == args.steps:
            _save_training_checkpoint(
                accelerator, args, output_dir, current_step,
                policy, optimizer, scheduler, ema, dataset,
                batch_buffer, nominal_epoch, _active_schema,
                _active_norm_stats, data_schemas, is_main,
            )
    ...
    gc.enable()
    if is_main:
        # Steps not divisible by log_freq leave the tail window only in memory —
        # flush it so the run end is reconstructable from logs/W&B.
        if loss_meter.count > 0:
            _final_msg = (
                f"Final partial window ({loss_meter.count} step(s)) | "
                f"loss: {loss_meter.avg:.4f} | "
                f"loss_action: {loss_action_meter.avg:.4f} | "
                f"grad_norm: {grad_norm_meter.avg:.4f} | "
                f"lr: {optimizer.param_groups[0]['lr']:.2e}"
            )
            logging.info(_final_msg)
            _metrics_file_logger().info(_final_msg)
            if wandb_logger:
                try:
                    wandb_logger.log({
                        "loss": loss_meter.avg,
                        "loss_action": loss_action_meter.avg,
                        "grad_norm": grad_norm_meter.avg,
                        "final_partial_window_n": loss_meter.count,
                    }, step=args.steps)
                except Exception as _e:  # noqa: BLE001
                    logging.debug(f"final wandb flush skipped: {_e}")
        if pbar:
            pbar.close()
