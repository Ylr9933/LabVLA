...
# Sibling modules that failed to import during autoload, as
# ``[(module_name, exception), ...]``. A failed import would otherwise make the
# broken schema look like an "unknown schema" downstream, masking the real
# ImportError root cause. The registry reads this list after autoload and fails
# loud (splicing the real exception) on the first ``resolve()`` /
# ``registered_names()`` call.
...
def _autoload_siblings() -> None:
    ...
    from src.schema import register, DatasetSchema  # local import to avoid cycles at package-init time
