...
> The scan/clean tooling here is v2.1-canonical. The 2026-04-20 "v3.0 dropped" decision applied to the OLD v3 SOURCE datasets (use `*_old`-suffix v2.1 copies under `/all-flash-data/lerobot/.cache/LabUtopia/` for scan/clean) — v3.0 repos themselves stay live for training: the v30 adapter, preflight, stats and the LabUtopia `*_v3` schemas consume them (M97).
...
Run `python -m data_process -h   # run from the repo root (module entry; per-file CLIs also self-bootstrap)` for the router help.
...
python -m data_process stats \
    ...
    [--chunk_size 50] \
    [--no-quantile]    # DEBUG ONLY: null out q01/q99 in the artifact (not faster —
                       # quantiles are still computed; default training/deploy REJECTS
                       # stats without q01/q99)
```

> **Which stats do the official launchers use? (M96)** The vlm/ki launchers bind sidecar canonical stats via `ExternalStatsMap` (`_canonical_stats_delta.json` for AgiBot, `stats_canonical_state_20260610.json` for OXE) and the finetune launcher requires `meta/stats_canonical_grip.json`. Recomputing `meta/stats.json` alone does NOT change what those launchers normalize with — regenerate the sidecar the launcher points at.
...
bash launch/vlm_pretrain/train_vlm_pretrain.sh  # edit DataRoot/DatasetSchema/ExternalStatsMap/Preflight at the top first
```
