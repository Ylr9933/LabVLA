...
Usage:
  python -m data_process migrate \\
    /path/to/pretrain_data/robointer_droid \\
    /path/to/data/LabUtopia/Level3_TransportBeaker \\
    --out schemas/
...
# __file__ = <repo>/data_process/migrate/manifest_to_py.py → repo root 2 up
sys.path.insert(0, str(Path(__file__).resolve().parent.parent.parent))

from src.schema.annotation_loss import AnnotationLossSpec  # noqa: E402
from src.schema.arm_layout import ArmCount, ArmLayoutSpec  # noqa: E402
from src.schema.dataset_schema import DatasetSchema  # noqa: E402
from src.schema.manifest import MANIFEST_NAME, load_manifest  # noqa: E402
...
def _has_extended_fields(schema: DatasetSchema) -> bool:
    ...
    return (
        ...
        or bool(schema.virtual_state_sources)
    )
...
def _detect_preset(schema: DatasetSchema) -> tuple[str, str]:
    ...
    if (
        ...
        if cams:
            ...
            body = (
                f"from src.schema.presets import franka8\n\n"
                f"SCHEMA = franka8(\n"
    ...
    if (
        ...
        body = (
            "from src.schema.presets import franka_split\n\n"
            "SCHEMA = franka_split(\n"
    ...
    body = (
        ...
        f"    virtual_state_sources={dict(schema.virtual_state_sources or {})!r},\n"
        '    source="manifest",\n'
