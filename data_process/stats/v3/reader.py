...
def read_action_state(ds_root: Path, schema, workers: int = 16):
    ...
    if next_frame_cols:
        raise NotImplementedError(
            "H28: schema declares next-frame synthetic action keys, but the "
            "v3 reader stacks multi-episode shards — a +1 shift would cross "
            "episode boundaries. Use per-episode (v2.1) data for this schema, "
            "or extend the v3 reader with episode-aware shifting."
        )
