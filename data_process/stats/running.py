...
def save(
    ...
    ``extra_meta``: additional top-level underscore-prefixed metadata
    keys (e.g. ``_layout_canonicalization: true`` when the stats were computed
    in CANONICAL layout space, so train-time
    ``transforms.core._canonicalize_single_arm_stats`` knows to skip its raw→
    canonical remap). Non-underscore keys are rejected — they would collide
    with feature entries.
    """
    path = Path(path)
    ...
    for k, v in (extra_meta or {}).items():
        if not str(k).startswith("_"):
            raise ValueError(
                f"save(extra_meta): key {k!r} must be underscore-prefixed "
                f"(top-level non-underscore keys are reserved for feature stats)."
            )
        if str(k) in payload:
            # Silently overwriting _chunk_size (or any already-written meta)
            # changes how the artifact is interpreted downstream.
            raise ValueError(
                f"save(extra_meta): key {k!r} would overwrite an existing "
                f"payload entry — reserved metadata cannot be clobbered."
            )
        payload[str(k)] = v
