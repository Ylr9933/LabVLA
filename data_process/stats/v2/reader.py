...
class OverwideColumnError(ValueError):
    ...
    and drops the parquet to keep its sample set in lock-step.

    The v3 stats reader does NOT unconditionally truncate —
    it passes ``truncate_overwide = (LABVLA_ALLOW_TRUNCATE == "1")`` (default
    False, i.e. this error RAISES), matching the adapter's ``_pad_row`` env
    gate so stats and training agree: both fail loud on overwide columns by
    default, both truncate only under the explicit escape hatch.
    """
    ...
    *, truncate_overwide: bool = False,
    ...
    if pa.types.is_fixed_size_list(t):
        ...
        if T == 0:
            ...
            if not truncate_overwide:
                # This scatter path silently drops values beyond
                # target_dim and zero-pads narrow rows — a ragged column would
                # produce "successful" stats while training fails loud on the
                # same data. Same escape hatch as everywhere else.
                raise OverwideColumnError(
                    f"ragged list column: per-row widths vary "
                    f"(min={int(lens.min())}, max={int(lens.max())}, "
                    f"target={target_dim}); refusing scatter-packing. Bind a "
                    "homogeneous physical/virtual column, or set "
                    "LABVLA_ALLOW_TRUNCATE=1 for the legacy behavior."
                )
...
def read_action_state(ds_root: Path, schema, workers: int = 32):
    ...
    (state_targets, action_targets, need_cols, read_state_keys,
     read_action_keys, next_frame_cols) = build_target_dims(schema)
    ...
    def _read_one(p: Path):
        try:
            ...
            if is_missing_column_error(e):
                return None
            raise
        # Overwide/ragged columns raise OverwideColumnError by default;
        # LABVLA_ALLOW_TRUNCATE=1 keeps stats and training truncation in sync.
        # The except below converts that into a PER-EPISODE drop — the same
        # episodes the v21 adapter's _parquet_ok filter excludes from
        # training — counted separately and reported honestly at the end.
        _stack = partial(
            _stack_column_pa,
        ...
        try:
            state = stack_keys_to_array(
                table, read_state_keys, state_targets, _stack
            )
            act = stack_keys_to_array(
                table, read_action_keys, action_targets, _stack,
                next_frame_shift=next_frame_cols,
            )
    ...
    if dropped_overwide:
        print(
            f"[stats] dropped {dropped_overwide} parquets with overwide/ragged "
            "columns (matches the v21 adapter's episode filter; set "
            "LABVLA_ALLOW_TRUNCATE=1 to truncate instead)"
        )
