...
Product: /path/to/pretrain_data/oxe-auge_clean/ (v3.0 LeRobot layout).
...
Usage:
    python -m data_process.merge_v3.build_oxe_auge_clean \\
        --src /path/to/pretrain_data/oxe-auge \\
        --dst /path/to/pretrain_data/oxe-auge_clean \\
...
def _scan_sub_repo(sub: Path) -> dict:
    ...
    # Record each sub-repo's declared observation.state width. The merge
    # canonicalizes ONLY observation.joints and carries the raw per-source
    # state through untouched (disk widths {2, 7, 15, 24}); recording the
    # declared width here surfaces that heterogeneity in the merge report
    # itself instead of leaving it discoverable only by a post-hoc disk audit
    # (audit_oxe_clean_v2 [5/5] cross-references the live distribution).
    state_shape = info.get("features", {}).get("observation.state", {}).get("shape", [None])
    common = {
        "sub_name": sub.name,
        "sub_path": sub,
        "info": info,
...
def write_info_json(
    ...
    info = {
        ...
        # Real deduplicated task count from merge_tasks, not the sub-repo count.
        "total_tasks": int(n_tasks) if n_tasks is not None else len(reports),
        "chunks_size": chunks_size,
        "data_files_size_in_mb": 500,
        "features": all_feats,
        # The merged tasks.parquet re-indexes tasks globally, but data shards
        # keep their per-source task_index values — adapters must not resolve
        # language through task_index on this product.
        "task_index_remapped": True,
    }
    if merge_provenance:
        # Pin what produced this dataset so identity cannot be silently reused
        # across semantically different merges.
        info["merge_provenance"] = dict(merge_provenance)
...
def write_labvla_manifest(dst_root: Path) -> None:
    ...
    The manifest is DERIVED from the registered ``schemas/oxe_auge.py`` schema
    and verified round-trip-equivalent after writing (fail-loud on any future
    drift). A manifest has discovery priority over the registry, so a
    hand-written one that re-bound state to raw ``observation.state`` under a
    different robot_type would silently resurrect broken state semantics.
    """
    import schemas.oxe_auge as _oxe_mod  # registers + exposes SCHEMA
    sch = _oxe_mod.SCHEMA
    _al = sch.arm_layout
    manifest = {
        "version": 1,
        "schema_id": sch.schema_id,
        "robot_type": sch.robot_type,
        "state": {"keys": list(sch.state_keys),
                  "dims": [int(d) for d in sch.state_dims]},
        "action": {
            "keys": list(sch.action_keys),
            "dims": [int(d) for d in sch.action_dims],
            "delta": [bool(b) for b in sch.delta_mask],
            "gripper_dims": [int(d) for d in sch.gripper_action_dims],
        },
        "virtual_state_sources": dict(sch.virtual_state_sources or {}),
        "gripper_semantic": sch.gripper_semantic,
        "images": dict(sch.image_mapping),
        "allow_extra_cameras": bool(sch.allow_extra_cameras),
        "arm_layout": {
            "arm_count": _al.arm_count.value,
            "arm_dof": int(_al.arm_dof),
            "gripper_index_in_raw": int(_al.gripper_index_in_raw),
        },
    }
    out = dst_root / "meta/labvla_manifest.json"
    out.write_text(json.dumps(manifest, indent=2))
    # Round-trip equivalence gate: what discovery will load from disk must be
    # the registered schema (modulo provenance fields).
    from src.schema.manifest import load_manifest
    loaded = load_manifest(out).to_dict()
    expected = sch.to_dict()
    for d in (loaded, expected):
        d.pop("source_path", None)
    if loaded != expected:
        _diff = {k for k in set(loaded) | set(expected)
                 if loaded.get(k) != expected.get(k)}
        raise RuntimeError(
            f"write_labvla_manifest: written manifest is NOT equivalent to "
            f"the registered oxe_auge schema (differing fields: {sorted(_diff)}). "
            f"Refusing to ship a manifest that would override the registry."
        )
...
def main():
    ...
    ap.add_argument("--allow-mixed-fps", action="store_true",
                    help="accept merging sources with different true fps under "
                         "one nominal fps (temporal resampling distortion; H3)")
    ap.add_argument(
    ...
    ap.add_argument(
        "--legacy-output-unlabeled-ok", action="store_true",
        help="H14: required when legacy flags (--no-apply-gripper-"
             "canonicalize / --no-whitelist-only) are used. The product then "
             "ships WITHOUT labvla_manifest.json (training must pass an "
             "explicit --dataset_schema) and carries merge_provenance in "
             "info.json, so legacy gripper semantics can never masquerade "
             "as the canonical oxe_auge_v1/open_fraction contract.")
    args = ap.parse_args()
    ...
    if args.dst.exists():
        if not args.allow_existing_dst:
            ...
            sys.exit(1)
        # A re-merge rewrites episodes/tasks/info/manifest but leaves the
        # previous meta/stats.json behind, and training prefers a repo's own
        # stats. Remove stale stats artifacts so a stats recompute is forced
        # for the new data composition.
        for _stale in ("meta/stats.json",):
            _p = args.dst / _stale
            if _p.exists():
                _p.unlink()
                logger.warning(
                    f"--allow-existing-dst: removed stale {_stale} (stats must "
                    f"be recomputed for the re-merged data; training would "
                    f"otherwise silently normalize with the OLD distribution)"
                )
    else:
    ...
    state_distrib = Counter(r["state_dim"] for r in reports)
    logger.info(
    ...
    # The merge does NOT canonicalize observation.state — surface its
    # per-source declared widths so a heterogeneous state column is a stated
    # fact of the merge report, not a post-hoc audit discovery.
    logger.info(f"state_dim distribution (NOT canonicalized): {dict(state_distrib)}")
    if len(state_distrib) > 1:
        logger.warning(
            f"HETEROGENEOUS observation.state — {len(state_distrib)} distinct "
            f"declared widths {sorted(k for k in state_distrib if k is not None)}. "
            f"The merged dataset's observation.state is NOT a usable canonical "
            f"state; schemas must bind state to a virtual copy of "
            f"observation.joints (see schemas/oxe_auge.py virtual_state_sources)."
        )
    if len(fps_distrib) > 1:
        if not getattr(args, "allow_mixed_fps", False):
            # A single nominal fps flattens distinct true sampling rates into
            # one fake time base — the adapter and all temporal semantics read
            # info.json["fps"].
            logger.error(
                f"MIXED FPS — {len(fps_distrib)} distinct values "
                f"{sorted(fps_distrib)} but the merged info.json can only "
                f"declare one fps={args.nominal_fps}. Refusing: non-matching "
                f"sub-repos would be silently temporally resampled. Re-run "
                f"with --allow-mixed-fps to accept that distortion explicitly."
            )
            sys.exit(1)
        logger.warning(
            f"MIXED FPS — {len(fps_distrib)} distinct values (explicitly "
            f"allowed via --allow-mixed-fps). Merged info.json declares "
            f"fps={args.nominal_fps}; adapter reads this. Non-matching "
            f"sub-repos effectively get temporal resampling."
        )
    ...
    # The merged tasks.parquet re-indexes tasks globally while data shards keep
    # per-source task_index values. Language must therefore resolve through
    # inline columns; refuse sources whose features expose NEITHER `task` NOR
    # `natural_language_instruction` (their frames would silently bind to the
    # wrong task text).
    _no_inline = [
        str(r["sub_path"].name) for r in reports
        if not ({"task", "natural_language_instruction"}
                & set((r["info"].get("features") or {}).keys()))
    ]
    if _no_inline:
        logger.error(
            f"{len(_no_inline)} source(s) expose no inline task column "
            f"(task / natural_language_instruction): {_no_inline[:8]}... "
            f"Their frames rely on per-source task_index, which the merged "
            f"global tasks.parquet re-indexes — language supervision would "
            f"bind to the WRONG task. Refusing to merge these sources."
        )
        sys.exit(1)

    _legacy_mode = (not args.apply_gripper_canonicalize) or (not args.whitelist_only)
    if _legacy_mode and not args.legacy_output_unlabeled_ok:
        logger.error(
            "H14: legacy flags (--no-apply-gripper-canonicalize / "
            "--no-whitelist-only) change the on-disk gripper/composition "
            "semantics, but the product would still be labeled as the "
            "canonical oxe_auge_v1 (gripper_semantic=open_fraction) contract. "
            "Pass --legacy-output-unlabeled-ok to proceed WITHOUT a "
            "labvla_manifest.json (explicit --dataset_schema then required "
            "for training)."
        )
        sys.exit(1)
    ...
    info = write_info_json(
        ...
        n_tasks=n_tasks,
        merge_provenance={
            "tool": "build_oxe_auge_clean",
            "whitelist_only": bool(args.whitelist_only),
            "apply_gripper_canonicalize": bool(args.apply_gripper_canonicalize),
            "nominal_fps": float(args.nominal_fps),
            "mixed_fps_allowed": bool(getattr(args, "allow_mixed_fps", False)),
            "n_sources": len(reports),
        },
    )
    if _legacy_mode:
        logger.warning(
            "H14 legacy mode: NOT writing labvla_manifest.json — schema "
            "discovery will not auto-bind oxe_auge_v1; pass an explicit "
            "--dataset_schema that matches the legacy semantics."
        )
    else:
