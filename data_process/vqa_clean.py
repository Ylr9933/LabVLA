    ...
    *,
    ...
    with invalid_path.open("w", encoding="utf-8") as invalid_handle:
        for source_rel in json_files:
            ...
            # Stamp the cleaned JSONL with the source's freshness fingerprint
            # (same sidecar format as scripts/build_vqa_jsonl_index.py) so
            # preflight can detect a stale/regenerated source instead of blindly
            # trusting offsets.
            from scripts.build_vqa_jsonl_index import write_source_fingerprint
            write_source_fingerprint(output_path, source_path)
