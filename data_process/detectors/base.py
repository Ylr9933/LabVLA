...
# One episode-mp4 filename pattern shared by every detector and kept consistent
# with the cleaner. Any digit width is accepted (some datasets use 4-digit
# episode numbers), so 4/5-digit datasets are not silently undetectable by scan
# yet processed by clean. The trailing `.mp4$` anchor keeps it precise.
EP_MP4_RE = re.compile(r"episode_(\d+)\.mp4$")
