...
Usage:
  ...
  python -m data_process clean \\
      --src    /path/to/pretrain_data/dataset \\
      --dst    /path/to/pretrain_data/dataset_clean \\
...
# Direct-file execution support (python data_process/cleanup/apply.py): repo
# root on sys.path before any data_process import below.
...
# Accept any digit width — some datasets use 4-digit episode numbers. Output is
# zfill(6)'d to the canonical 6-digit form. Detectors share the same any-width
# policy (data_process/detectors/base.py::EP_MP4_RE); keep the two in lockstep,
# else a width mismatch makes scan blind to files that clean would process.
...
def rewrite_meta(
    ...
    # Removing orphan episodes_stats rows also changes what the aggregate stats
    # describe, so treat that as a distribution change too.
    _distribution_changed = len(drop_eps) > 0 or len(orphan_stats_eps) > 0
    if stats_src.exists():
        if preserve_stats:
...
def main():
    ...
    ap.add_argument("--clear-dst", action="store_true",
                    help="Delete dst contents before re-emitting. Pairs "
    ...
    ap.add_argument("--preserve-stats", action="store_true",
                    help="Keep the source stats.json verbatim. Default is to mark "
                         "stats invalidated after cleaning so downstream callers "
                         "regenerate them.")
    ap.add_argument("--allow-report-source-mismatch", action="store_true",
                    help="Apply a scan report generated for a DIFFERENT "
                         "source path (dangerous; episode ids may not align).")
    ap.add_argument("--allow-incompatible", action="store_true",
                    help="Proceed even when the scan report flags the dataset as "
                         "model-incompatible (e.g. action/state dim exceeds the "
                         "model cap). Default refuses, because a per-episode "
                         "symlink-clean cannot fix a dataset-wide dimension "
                         "violation.")
    ...
    if args.report:
        ...
        # Also refuse a report that does not belong to this --src (stale copy
        # / wrong dataset) unless explicitly overridden.
        _binding = report.get("source_binding") or {}
        _rep_root = _binding.get("root_resolved") or report.get("root")
        if _rep_root and str(Path(_rep_root).resolve()) != str(args.src.resolve()):
            if not getattr(args, "allow_report_source_mismatch", False):
                raise SystemExit(
                    f"[M49] report was generated for {_rep_root!r} but --src is "
                    f"{args.src.resolve()!s} — refusing to apply a foreign "
                    f"drop list (pass --allow-report-source-mismatch to "
        ...
        # The manual path must not bypass the scan-time dataset-compatibility
        # gate: v3 layouts have no episodes.jsonl and would otherwise die on a
        # bare FileNotFoundError deep inside.
        from data_process.meta_reader.registry import detect_codebase_version
        _ver = detect_codebase_version(args.src)
        if not str(_ver).startswith("v2"):
            raise SystemExit(
                f"[M60] clean only supports LeRobot v2.x layouts (detected "
                f"{_ver!r}); v3 packed layouts have no episodes.jsonl and are "
                f"cleaned via merge/preflight tooling instead."
            )
    ...
    # With src == dst, --clear-dst would delete the SOURCE's
    # data/videos/images/meta before reading its meta — real data destruction,
    # violating the "src is NEVER modified" contract.
    _src_r, _dst_r = args.src.resolve(), args.dst.resolve()
    if _src_r == _dst_r:
        raise SystemExit(
            f"--dst resolves to the SOURCE dataset ({_src_r}); refusing — "
            "clean always emits into a separate directory (src is never "
            "modified). Pick a fresh --dst."
        )
    if _dst_r in _src_r.parents:
        raise SystemExit(
            f"--dst {_dst_r} contains the source {_src_r}; --clear-dst would "
            "delete managed subdirs overlapping the source tree. Pick a "
    ...
    # A stale report / typo'd --drop-episodes must not fabricate "successful"
    # deletions of ids that never existed — the real data would be unchanged
    # while logs claim dropped>0 and stats get needlessly invalidated.
    _all_eps = set(kept_old)
    with open(src_meta / "episodes.jsonl") as _f:
        for _line in _f:
            _line = _line.strip()
            if _line:
                _all_eps.add(int(json.loads(_line)["episode_index"]))
    _unmatched = sorted(e for e in drop_eps if e not in _all_eps)
    if _unmatched:
        raise SystemExit(
            f"[M81] {len(_unmatched)} drop id(s) do not exist in the source "
            f"dataset (first 10: {_unmatched[:10]}). The report/--drop-episodes "
            f"list does not belong to this dataset — refusing a no-op 'clean'."
        )
    drop_eps = {e for e in drop_eps if e in _all_eps}
    logger.info("[M81] drop ids validated: requested=%d actual=%d",
                len(_unmatched) + len(drop_eps), len(drop_eps))
