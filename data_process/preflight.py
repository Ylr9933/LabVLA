...
Usage:
    python -m data_process preflight \\
        ...
        --external_stats_map "RoboCOIN=/path/to/pretrain_data/robocoin_clean/meta/stats.json"
...
def _check_vqa_manifest(repo_id: str, root: Path) -> list[ValidationIssue]:
    ...
    for rel in json_files:
        ...
        if rel.endswith(".jsonl"):
            # When a source fingerprint sidecar exists, verify the JSONL is
            # still fresh w.r.t. its source (size+mtime); a regenerated source
            # with a stale cleaned/indexed JSONL must not pass the gate.
            _fp_path = Path(f"{meta_path}.source.json")
            if _fp_path.is_file():
                try:
                    import json as _json
                    _fp = _json.loads(_fp_path.read_text())
                    _src = Path(str(_fp.get("source", "")))
                    if not _src.is_file():
                        bad_jsonl_offsets.append(
                            f"{rel}: fingerprint source missing: {_src}")
                    else:
                        _st = _src.stat()
                        if (int(_st.st_size) != int(_fp.get("size", -1))
                                or int(_st.st_mtime_ns) != int(_fp.get("mtime_ns", -1))):
                            bad_jsonl_offsets.append(
                                f"{rel}: STALE vs source {_src.name} "
                                f"(size/mtime changed since indexing — "
                                f"re-run vqa_clean / build_vqa_jsonl_index)")
                except Exception as _e:  # noqa: BLE001
                    bad_jsonl_offsets.append(f"{rel}: unreadable fingerprint: {_e}")
...
def _check_stats_resolution(
    ...
    try:
        ...
        return issues
    # Require the blocks the chosen action_mode actually consumes, not just
    # "any canonical key".
    _required = ["observation.state"]
    _required.append("action_abs" if action_mode == "abs" else "action")
    _missing_req = [c for c in _required if not isinstance(parsed.get(c), dict)]
    if _missing_req:
        issues.append(ValidationIssue(
            severity="HIGH", repo=repo_id,
            message=(
                f"stats.json lacks required block(s) {_missing_req} for "
                f"action_mode={action_mode!r} (training normalize/delta needs "
                f"them; M30)"
            ),
            context={"path": str(resolved.path),
        ...
        return issues
    # Stats horizon must match the training chunk size when declared.
    if expected_chunk_size is not None:
        _cs = parsed.get("_chunk_size")
        if _cs is None:
            issues.append(ValidationIssue(
                severity="MED", repo=repo_id,
                message=(f"stats.json carries no _chunk_size marker; cannot "
                         f"verify against --chunk_size={expected_chunk_size} "
                         f"(legacy artifact; M8)"),
                context={"path": str(resolved.path)},
            ))
        elif int(_cs) != int(expected_chunk_size):
            issues.append(ValidationIssue(
                severity="HIGH", repo=repo_id,
                message=(f"stats _chunk_size={int(_cs)} != --chunk_size="
                         f"{int(expected_chunk_size)} (training dataset build "
                         f"will fail on this; M8)"),
                context={"path": str(resolved.path)},
            ))
            return issues
    # Key presence is not usability — every stats field that exists must be a
    # 1-D finite numeric list (training consumes them as arrays).
    for _ck in ("observation.state", "action", "action_abs"):
        _blk = parsed.get(_ck)
        if not isinstance(_blk, dict):
            continue
        for _fld in ("mean", "std", "q01", "q99", "min", "max"):
            _v = _blk.get(_fld)
            if _v is None:
                continue
            if (not isinstance(_v, list) or not _v
                    or not all(isinstance(x, (int, float)) for x in _v)):
                issues.append(ValidationIssue(
                    severity="HIGH", repo=repo_id,
                    message=(
                        f"stats[{_ck!r}][{_fld!r}] is not a non-empty numeric "
                        f"list (got {type(_v).__name__}; M107) — training "
                        f"would mis-broadcast or crash at hydrate"
                    ),
                    context={"path": str(resolved.path)},
                ))
                return issues
...
def main() -> None:
    ...
    parser.add_argument("--json", type=str, default=None,
                        help="L39: also write the structured issue list "
                             "(incl. context payloads) to this JSON file.")
    parser.add_argument("--chunk_size", type=int, default=None,
                        help="Training chunk size (M8): when given, every "
                             "resolved stats.json must carry a matching "
                             "_chunk_size (training fails on mismatch at "
                             "dataset build; catch it here).")
    ...
    if ext_map:
        _unknown_keys = sorted(set(ext_map) - set(args.repos))
        if _unknown_keys:
            raise SystemExit(
                f"[preflight/M48] --external_stats_map repo key(s) "
                f"{_unknown_keys} are not in --repos {sorted(args.repos)} — "
                f"the override would silently miss."
            )
    ...
    if args.schema:
        try:
            from src.schema import resolve as _resolve_schema
            default_schema = _resolve_schema(args.schema)
        except Exception as e:
            # An EXPLICIT --schema is an operator instruction — degrading to
            # "schema-aware checks disabled" lets a typo'd contract pass.
            raise SystemExit(
                f"[preflight/M29] --schema={args.schema!r} failed to resolve: "
                f"{e!r}. An explicit schema must bind; fix the spec."
            )
    if args.schema_per_repo:
        try:
            ...
            # All-or-nothing — a half-applied map silently downgrades the
            # remaining repos to auto-discover.
            raise SystemExit(
                f"[preflight/M29] --schema_per_repo failed to parse/resolve: "
                f"{e!r}. Explicit per-repo schema bindings must ALL resolve."
            )
    ...
    for r, root in repo_roots.items():
        ...
        if repo_schema is None:
            try:
                from src.schema import discover_schema as _discover_schema
    ...
    if getattr(args, "json", None):
        ...
        with open(args.json, "w") as _jf:
            json.dump([_dc.asdict(i) for i in all_issues], _jf, indent=2)
        logging.info("structured issues written to %s", args.json)
