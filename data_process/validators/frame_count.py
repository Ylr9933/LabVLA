...
class FrameCountValidator(Validator):
        ...
        for repo in repos:
            ...
            if not ep_jsonl.exists():
                if is_v3:
                    ...
                    # v3 still gets the free meta-arithmetic check
                    # (length == dataset_to_index - dataset_from_index for
                    # every episode) — the invariant adapter indexing rests on.
                    issues.extend(self._check_v3_length_arithmetic(repo_path))
                else:
                    issues.append(ValidationIssue(
                        severity="HIGH",
                        repo=str(repo_path),
            ...
            # chunks_size for the v2.1 per-episode parquet row check.
            chunks_size = 1000
            try:
                with open(repo_path / "meta" / "info.json") as _f:
                    chunks_size = int(json.load(_f).get("chunks_size", 1000))
            except Exception:
                pass
            ...
            for line in sampled:
                ...
                # Cross-check the parquet row count against the meta length
                # (the adapter samples frame_in_ep ∈ [0, length) and indexes the
                # parquet directly — length > rows crashes mid-training, length <
                # rows silently drops tail frames). Metadata-only read (no column
                # data), cheap per sampled episode.
                if not is_v3:
                    pq_path = (
                        repo_path / "data" / f"chunk-{ei // chunks_size:03d}"
                        / f"episode_{ei:06d}.parquet"
                    )
                    if pq_path.is_file():
                        try:
                            import pyarrow.parquet as _pq
                            n_rows = int(_pq.ParquetFile(str(pq_path)).metadata.num_rows)
                        except Exception as e:
                            issues.append(ValidationIssue(
                                severity="HIGH",
                                repo=str(repo_path),
                                message=(
                                    f"episode {ei}: parquet metadata unreadable "
                                    f"for row-count check: {e}"
                                ),
                                context={"episode_index": ei, "parquet": str(pq_path)},
                            ))
                            n_rows = None
                        if n_rows is not None and n_rows != expected:
                            issues.append(ValidationIssue(
                                severity="HIGH",
                                repo=str(repo_path),
                                message=(
                                    f"episode {ei}: episodes.jsonl length="
                                    f"{expected} but parquet has {n_rows} rows "
                                    f"(diff={n_rows - expected}) — adapter "
                                    f"indexing would crash or silently drop "
                                    f"tail frames"
                                ),
                                context={
                                    "episode_index": ei,
    ...
    def _check_v3_length_arithmetic(self, repo_path: Path) -> list[ValidationIssue]:
        """Verify ``length == dataset_to_index - dataset_from_index`` for EVERY
        v3 episode (pure metadata arithmetic, no shard I/O).

        A longer meta length makes the training adapter IndexError mid-run; a
        shorter one silently drops tail frames from sampling. The v30 adapter
        now fail-louds on the same invariant at init; this validator surfaces
        it at preflight time instead.
        """
        issues: list[ValidationIssue] = []
        ep_root = repo_path / "meta" / "episodes"
        ...
        try:
            ...
            total = 0
            for p in sorted(ep_root.rglob("*.parquet")):
                t = _pq.read_table(
                    p,
                    columns=["episode_index", "length",
                             "dataset_from_index", "dataset_to_index"],
                )
                eis = t.column("episode_index").to_pylist()
                lens = t.column("length").to_pylist()
                froms = t.column("dataset_from_index").to_pylist()
                tos = t.column("dataset_to_index").to_pylist()
                total += len(eis)
                for ei, ln, a, b in zip(eis, lens, froms, tos):
                    if int(ln) != int(b) - int(a):
                        bad.append((int(ei), int(ln), int(b) - int(a)))
        except Exception as e:
            issues.append(ValidationIssue(
                severity="MED",
                repo=str(repo_path),
                message=f"v3 length-arithmetic check unreadable: {e}",
                context={"is_v3": True},
            ))
            return issues
        if bad:
            sample = ", ".join(
                f"ep{ei}: length={ln} vs to-from={rng}" for ei, ln, rng in bad[:5]
            )
            issues.append(ValidationIssue(
                severity="HIGH",
                repo=str(repo_path),
                message=(
                    f"{len(bad)}/{total} v3 episode(s) violate length == "
                    f"dataset_to_index - dataset_from_index ({sample}"
                    f"{' ...' if len(bad) > 5 else ''}) — adapter indexing "
                    f"would crash or silently drop tail frames"
                ),
                context={"is_v3": True, "bad_count": len(bad), "total": total},
            ))
        return issues
    ...
    def _build_v3_video_shard_map(
        ...
        the legacy lerobot v2.1 video-path resolution; the live counterpart is
        src/adapters/lerobot_v21.py). Without it,
