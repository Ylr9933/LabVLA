...
class LabVLALabUtopiaPolicy:
    ...
    def __init__(
        ...
        if self._norm_stats is not None:
            ...
            # (use_delta_action may flip there, so the abs-entry presence check
            # must happen post-load — see below).
        ...
        if self._norm_stats is not None and self._schema is not None:
            ...
            if _nm_check is not None:
                _resolved = self._resolve_action_stat_keys(_nm_check)
                logger.info(
                    "serve_labvla: action denormalization will read stats "
                    "entries %r (use_delta_action=%s).",
                    _resolved, self.use_delta_action,
                )
    ...
    def _load_policy(self, pretrained_path: str, vlm_path: str):
        ...
        # Self-contained checkpoint: if it bundles the Qwen3-VL assets FLAT in
        # pretrained_model/ (signalled by vlm_config.json — the renamed Qwen arch config),
        # point vlm_path at that dir so BOTH the backbone build (weightless from_config, via
        # saved_fwd["vlm_pretrained_path"]) AND the processor load
        # (Qwen3VLProcessor.from_pretrained(vlm_path)) read from the checkpoint — no external
        # Qwen base / --vlm_path required.
        for _cand_dir in (ckpt_root / "pretrained_model", ckpt_root):
            if (_cand_dir / "vlm_config.json").is_file():
                logger.info(
                    f"[self-contained] using bundled VLM assets in {_cand_dir} "
    ...
    # checkpoint's training domain: an abs-mode ckpt inverted with delta-domain
    # q01/q99 mis-scales actions ~8x (measured on agibot stats).
    _ABS_KEY_ALIASES = {
        "action": ("action_abs", "actions_abs"),
        "actions": ("actions_abs", "action_abs"),
    }

    def _resolve_action_stat_keys(self, stats_dict: dict) -> list[str]:
        """Pick the per-schema-key stats entries matching the TRAINING domain.
        ...
        resolved: list[str] = []
        for k in keys:
            candidates = self._ABS_KEY_ALIASES.get(k, (f"{k}_abs",))
            hit = next((c for c in candidates if c in stats_dict), None)
            if hit is None:
                raise RuntimeError(
                    f"serve_labvla: action_mode=abs requires absolute-domain "
                    f"action stats ({candidates!r} for schema key {k!r}) but "
                    f"norm_stats only has {sorted(stats_dict.keys())[:12]!r}. "
                    f"Inverting with the delta-domain 'action' entry would "
                    f"mis-scale physical actions (~8x on agibot). Re-save the "
                    f"checkpoint with a stats.json that carries 'action_abs' "
                    f"(any `python -m data_process stats` output does)."
                )
            resolved.append(hit)
        return resolved
